...
describe('ProcessOrchestrator', () => {
    ...
    describe('stopProcess()', () => {
        ...
        it('should be idempotent and handle non-existent processes gracefully', async () => {
            ...
            vi.mocked(pm2.delete).mockImplementation((name, cb) => cb(new Error('process name not found')));
    ...
    describe('startProcess()', () => {
        it('should sanitize the project name, connect to PM2, and start the process', async () => {
            ...
            vi.mocked(pm2.start).mockImplementation((options, cb) => cb(null, [{ pm_id: 123, name: expectedPm2Name }]));
        ...
        it('should throw an error if pm2 connection fails', async () => {
            vi.mocked(pm2.connect).mockImplementation((cb) => cb(new Error('PM2 Connect Fail')));
        ...
        it('should throw an error if pm2 start fails', async () => {
            vi.mocked(pm2.connect).mockImplementation((cb) => cb(null));
            vi.mocked(pm2.list).mockImplementation((cb) => cb(null, []));
            vi.mocked(pm2.start).mockImplementation((options, cb) => cb(new Error('PM2 Start Fail')));
    ...
    describe('listProcesses()', () => {
        it('should return a list of all managed processes', async () => {
            vi.mocked(pm2.connect).mockImplementation((cb) => cb(null));
            vi.mocked(pm2.list).mockImplementation((cb) => cb(null, []));
            vi.mocked(pm2.start).mockImplementation((options, cb) => cb(null, [{ pm_id: 1, name: 'mock' }]));
