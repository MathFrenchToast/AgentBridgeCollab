...
describe('McpBridge', () => {
    ...
    it('should forward stdout logs to the correct project server if it is JSON-RPC', async () => {
        ...
        await bridge.initializeProjectServer(projectId);
        // Get the on handler
        const onCall = mockOrchestrator.on.mock.calls.find((call) => call[0] === 'LOG_EMITTED');
    ...
    describe('lifecycle notifications', () => {
        it('should send a notification when PROCESS_ONLINE is emitted', () => {
            const projectId = 'test-project';
            const channelId = 'channel-123';
            mockOrchestrator.on.mock.calls.find((call) => call[0] === 'PROCESS_ONLINE')[1]({ projectId, channelId });
        ...
        it('should send a notification when PROCESS_EXITED is emitted', () => {
            const projectId = 'test-project';
            const channelId = 'channel-123';
            mockOrchestrator.on.mock.calls.find((call) => call[0] === 'PROCESS_EXITED')[1]({ projectId, channelId });
        ...
        it('should send a notification when PROCESS_CRASHED is emitted', () => {
            const projectId = 'test-project';
            const channelId = 'channel-123';
            mockOrchestrator.on.mock.calls.find((call) => call[0] === 'PROCESS_CRASHED')[1]({ projectId, channelId });
