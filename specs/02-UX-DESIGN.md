# UX Design: Agent Bridge Collaboration (ABC)
...
| :--- | :--- | :--- |
| `/start <id> <prompt>` | Creates a new channel and starts a agent process. | `/start refactor-auth "Check all files in /src/auth for security issues"` |
| `/stop <id>` | Gracefully stops the specified agent process and deletes the channel. | `/stop refactor-auth` |
| `/status <id>` | Shows the current status (Running, Waiting for Input, Crashed) of a process. | `/status refactor-auth` |
| `/list` | Lists all active ABC processes and their associated channels. | `/list` |

## 3. Dedicated Project Space (e.g., Discord Channel)
When a project starts, ABC creates a dedicated channel.
